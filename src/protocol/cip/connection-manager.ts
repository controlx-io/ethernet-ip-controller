 ...
 */
const build_connectionParameters = (
  owner: number,
  type: number,
  priority: number,
  fixedVar: number,
 ...
 */
const build_forwardOpen = (
  otRPI: number = 8000,
  netConnParams: number = 0x43f4,
  timeOutMs: number = 1000,
  timeOutMult: number = 32,
  connectionSerial: number = 0x4242,
  TOconnectionID: number = getRandomInt(2147483647),
): Buffer => {
  ...
  const actualMultiplier =
    timeOutMultiplier[timeOutMult as keyof typeof timeOutMultiplier];
 ...
 */
const build_forwardClose = (
  timeOutMs: number = 1000,
  vendorOrig: number = 0x3333,
  serialOrig: number = 0x1337,
  connectionSerial: number = 0x4242,
): Buffer => {
  if (timeOutMs <= 900 || typeof timeOutMs !== "number") {
