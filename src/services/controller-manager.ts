...
class ControllerManager {
  ...
  addController(
    ...
    const contLength = this.controllers.push(
      new extController(ipAddress, slot, rpi, connected, retrySP, opts),
    );
  ...
  getAllValues(): cmAllControllersValues {
    const allTags: cmAllControllersValues = {};
    this.controllers.forEach((controller) => {
      const tags: cmAllValues = {};
...
export class extController extends EventEmitter implements IEextController {
  reconnect: boolean;
  ipAddress: string;
  slot: number | Buffer;
  opts: any;
  ...
  addTag(
    ...
    const tagItem = this.tags.find((tag) => {
   ...
   * @param program - Program Name
   */
  removeTag(tagname: string, program: string | null = null) {
