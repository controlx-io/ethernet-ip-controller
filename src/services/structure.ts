...
import Template from "./structure-template.ts";
import { Buffer } from "node:buffer";
import { bufferToString, stringToBuffer } from "../utils/utilities.ts";
import equals from "deep-equal";
import type TagList from "./tag-list.ts";
...
export class Structure extends Tag {
   ...
   */
  override set value(newValue: any) {
    if (!this._template) {
  ...
  parseValue(data: Buffer): any {
    if (this.state.tag.arrayDims > 0) {
      ...
      return this._parseReadData(data, this._template as Template);
    }
  }
  ...
  writeObjToValue(): void {
    if (this.state.tag.arrayDims > 0) {
      ...
      super.value = this._parseWriteData(
        this._valueObj,
   ...
   */
  override generateWriteMessageRequest(
    value = null,
    size: number = 0x01,
  ): Buffer {
    ...
    if (!this._template) {
      ...
      buf.writeUInt16LE(
        this._template._attributes.StructureHandle as number,
        2,
      );
      ...
      return MessageRouter.build(
        WRITE_TAG,
        tag.path,
        Buffer.concat([buf, super.value]),
      );
    }
  }
   ...
   * @returns message
   */
  override generateWriteMessageRequestFrag(
    offset: number = 0,
    ...
    if (!this._template) {
      ...
      return MessageRouter.build(
        ...
        tag.path,
        Buffer.concat([buf, value ?? Buffer.alloc(0)]),
      );
    }
  }
  ...
  _parseReadData(data: Buffer, template: Template): any {
    if (
      template._members.length === 2 && template._members[0].name === "LEN" &&
      template._members[1].name === "DATA"
    ) {
    ...
    const { SINT, INT, DINT, REAL, LINT, BIT_STRING, BOOL, STRUCT } =
      CIP.DataTypes.Types;
      ...
      switch (member.type.structure ? STRUCT : member.type.code) {
        ...
        case BIT_STRING:
          if (member.type.arrayDims > 0) {
            const array = [];
            for (let i = 0; i < member.info * 4; i += 4) {
              const bitString32bitValue = data.readUInt32LE(member.offset + i);
              for (let j = 0; j < 32; j++) {
        ...
        case BOOL:
          structValues[member.name] =
            !!(data.readUInt8(member.offset) & (1 << member.info));
          break;
        case STRUCT: {
          ...
          if (member.type.arrayDims > 0) {
            ...
            for (
              let i = 0;
              i < member.info * memberStructSize;
              i += memberStructSize
            ) {
              array.push(
                this._parseReadData(
                  data.subarray(member.offset + i),
                  memberTemplate,
                ),
              );
            }
            ...
            structValues[member.name] = this._parseReadData(
              data.subarray(member.offset),
              memberTemplate,
            );
          }
          break;
  ...
  _parseReadDataArray(data: Buffer): any[] {
    ...
    for (
      let i = 0;
      i < data.length;
      i += this._template?._attributes.StructureSize as number ?? 0
    ) {
      array.push(
        this._parseReadData(
          data.subarray(i),
  ...
  _parseWriteData(structValues: any, template: Template): Buffer {
    if (
      ...
      return stringToBuffer(
        structValues,
        template._attributes.StructureSize as number,
      );
    }

    const data = Buffer.alloc(template._attributes.StructureSize as number);
    ...
    template._members.forEach((member) => {
      ...
      switch (member.type.structure ? STRUCT : member.type.code) {
          ...
          if (member.type.arrayDims > 0) {
            for (let i = 0; i < member.info; i++) {
              if (typeof structValues[member.name][i] == "bigint") {
                ...
                data.writeBigInt64LE(
                  BigInt(structValues[member.name][i]),
                  member.offset + (i * 8),
                );
              }
            }
            ...
            if (typeof structValues[member.name] == "bigint") {
              ...
              data.writeBigInt64LE(
                BigInt(structValues[member.name]),
                member.offset + 8,
              );
            }
          }
        ...
        case STRUCT: {
          ...
          if (member.type.arrayDims > 0) {
            for (let i = 0; i < member.info; i++) {
              const templateData = this._parseWriteData(
                structValues[member.name][i],
                memberTemplate,
              );
              for (const pairs of templateData.entries()) {
                data[member.offset + (i * memberStructSize) + pairs[0]] =
                  pairs[1];
              }
            }
          } else {
  ...
  _parseWriteDataArray(newValue: any[]) {
    ...
    newValue.forEach((value) => {
      buf = Buffer.concat([
        buf,
        this._parseWriteData(value, this._template as Template),
      ]);
    });
   ...
   */
  override get controller_value(): Buffer {
   ...
   */
  override set controller_value(newValue: any) {
    if (!equals(newValue, this.state.tag.controllerValue)) {
      ...
      if (lastValue !== null) {
        this.emit("Changed", this, this.parseValue(lastValue));
