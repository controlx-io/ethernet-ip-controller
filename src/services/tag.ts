...
import {
  ...
  isValidTypeCode,
  Types,
} from "../protocol/cip/data-types.ts";
import { dateFormat } from "../utils/utilities.ts";
...
export interface ITag {
  ...
  on(event: "Initialized", listener: (this: this) => void): this;
  on(event: "Unknown", listener: (this: this) => void): this;
  on(event: "KeepAlive", listener: (this: this) => void): this;
}

export class Tag extends EventEmitter implements ITag {
  state: tagState;
  ...
  constructor(
    ...
    if (datatype !== null && !isValidTypeCode(datatype)) {
    ...
    if (keepAlive < 0) {
      throw new Error(
        `Tag expected keepAlive to be greater than 0, got ${keepAlive}`,
      );
    }
  ...
  static parsePath(
    ...
    const pathArr = tagname.split(/[.[\],]/).filter((segment: string) =>
      segment.length > 0
    );

    let bitIndex: number | null = null;
    ...
    const memArr = tagname.split(".");
    const isBitIndex = (memArr.length > 1) &&
      !isNaN(Number(memArr[memArr.length - 1]));

    // Check if BIT_STRING data type was passed in
    const isBitString = datatype === Types.BIT_STRING &&
      !isNaN(Number(pathArr[pathArr.length - 1]));
    ...
    if (isBitString) {
      ...
      if (isBitIndex) {
        ...
        bitIndex = parseInt(pathArr.pop() as string);
        if ((bitIndex < 0) || (bitIndex > 31)) {
          throw new Error(
            `Tag bit index must be between 0 and 31, received ${bitIndex}`,
          );
        }
      }
    }
    ...
    if (program) {
      bufArr.push(CIP.EPATH.segments.DATA.build(`Program:${program}`));
    }
  ...
  get value(): any {
    if (Array.isArray(this.state.tag.value)) {
      const prevValue = [...this.state.tag.value];
      setTimeout(() => {
  ...
  set controller_value(newValue: any) {
    if (!equals(newValue, this.state.tag.controllerValue)) {
      ...
      if (this.state.keepAlive > 0) {
        const now = new Date();
        if (
          now.getTime() - this.state.timestamp.getTime() >=
            this.state.keepAlive * 1000
        ) {
          this.state.tag.controllerValue = newValue;
   ...
   */
  get error(): tagError {
    return this.state.error.code ? this.state.error : { code: 0, status: null };
  }
   ...
   */
  generateReadMessageRequest(size: number | null = null): Buffer {
   ...
   */
  generateReadMessageRequestFrag(
    offset: number = 0,
  ...
  parseReadMessageResponseValueForBitIndex(data: Buffer) {
    ...
    switch (this.state.tag.type) {
      case SINT:
        this.controller_value = (data.readInt8(2) & (1 << tag.bitIndex)) == 0
          ? false
          : true;
        break;
      case UINT:
        ...
        break;
      case INT:
        this.controller_value = (data.readInt16LE(2) & (1 << tag.bitIndex)) == 0
          ? false
          : true;
        break;
      case DINT:
      case BIT_STRING:
        this.controller_value = (data.readInt32LE(2) & (1 << tag.bitIndex)) == 0
          ? false
          : true;
        break;
      default:
      ...
      case SHORT_STRING: // single byte character string
        if (data.length > 3) {
          ...
          if (len !== this.controller_value.length) {
            throw new Error(
              ` Read from Controller: Short string length mismatch`,
            );
          }
        } else {
  ...
  generateWriteMessageRequest(value: any = null, size: number = 0x01): Buffer {
    ...
    if (tag.type === null) {
      throw new Error(
        `Tag ${tag.name} has not been initialized. Try reading the tag from the controller first or manually providing a valid CIP datatype.`,
      );
    }
    ...
    switch (tag.type) {
      ...
      case SHORT_STRING:
        ...
        if (!tag.value || tag.value.length === 0) {
          ...
          if (strlen > 255) {
            throw new Error(
              `String too long to Write to Controller: ${tag.type}, limit 255 bytes, length ${strlen}`,
            );
          }
      ...
      default:
        throw new Error(
          `Unrecognized Type to Write to Controller: ${tag.type}`,
        );
    }
   ...
   * @memberof Tag
   */
  generateWriteMessageRequestFrag(
    offset: number = 0,
    value: any = null,
    size: number = 0x01,
  ): Buffer {
  ...
  static isValidTagname(tagname: string): boolean {
    ...
    // check segments
    if (
      tagname.split(/[:.[\],]/).filter((segment) => segment.length > 40)
        .length > 0
    ) {
