...
import type Tag from "../services/tag.ts";
...
class TagGroup {
   ...
   */
  forEach(callback: (tag: Tag) => Promise<void> | void) {
  ...
  generateReadMessageRequests(): readMessageRequests[] {
    ...
    const messages: readMessageRequests[] = [];
    let msgArr: Buffer[] = [];
    let tagIds: string[] = [];
    let messageLength = 0;
    ...
    for (const key of Object.keys(tags)) {
      ...
      const msg = tag.generateReadMessageRequest();
      ...
      if (messageLength >= 300) {
        ...
        buf = Buffer.concat([buf, ...msgArr as Buffer[]]);
  ...
  generateWriteMessageRequests(): writeTagMessageRequests[] {
    ...
    // Initialize Variables
    const messages: writeTagMessageRequests[] = [];
    let msgArr: Buffer[] = [];
    let tagIds: string[] = [];
    let messageLength = 0;
    ...
    for (const key of Object.keys(tags)) {
      ...
      if (
        tag.value !== null && tag.type === "STRUCT" && tag instanceof Structure
      ) {
        tag.writeObjToValue();
      }

      if (
        tag.value !== null && !equals(tag.state.tag.value, tag.controller_value)
      ) {
        // Build Current Message
        const msg = tag.generateWriteMessageRequest();
        ...
        if (tag.type !== "STRUCT") {
          ...
          if (messageLength >= 350) {
            ...
            messages.push({ data, tag: null, tag_ids: tagIds });
            messageLength = 0;
            msgArr = [];
            tagIds = [];
          }
        } else {
          messages.push({ data: null, tag: tag, tag_ids: null }); // Single tag pushed to indicate need to write single STRUCT tag
        }
      }
    }
