...
import { CIP, ENIP } from "../protocol/enip.ts";
import type { ENIPState } from "../protocol/enip.ts";
import TagGroup from "./tag-group.ts";
import { dateFormat, delay, promiseTimeout } from "../utils/utilities.ts";
import TagList, { type tagListTag, type tagListTemplates } from "./tag-list.ts";
import { Structure } from "./structure.ts";
import { TaskEasy as Queue } from "task-easy";
import Tag from "./tag.ts";
import type { CommonPacketData } from "../protocol/cip/encapsulation.ts";
import { CIPError } from "../protocol/cip/errors.ts";

export interface IPriorityCompare {
  priority: number;
  timestamp: Date;
}

const compare = (obj1: IPriorityCompare, obj2: IPriorityCompare) => {
...
interface ExtendedENIPState extends ENIPState {
...
class Controller extends ENIP {
   ...
   */
  get time(): string {
    if (!this.state.controller.time) return "";
    return dateFormat(
   ...
   */
  async getAttributeSingle(
    classID: number,
    instance: number,
    attribute: number,
    attData: Buffer = Buffer.alloc(0),
  ): Promise<Buffer> {
  ...
  async forwardOpen(): Promise<number> {
    ...
    const portPath = Buffer.concat([
      this.state.controller.path as Buffer,
      mrPath,
    ]);
  ...
  override write_cip(
    ...
    if (connected === false) {
      msg = UnconnectedSend.build(
        data,
        this.state.controller.path as Buffer,
        this.state.unconnectedSendTimeout,
      );
    } else {
      msg = data;
    }
  ...
  async readControllerProps(): Promise<void> {
    ...
    this.state.controller.minorRecoverableFault = (status & 0x0100) === 0
      ? false
      : true;
    this.state.controller.minorUnrecoverableFault = (status & 0x0200) === 0
      ? false
      : true;
    this.state.controller.majorRecoverableFault = (status & 0x0400) === 0
      ? false
      : true;
    this.state.controller.majorUnrecoverableFault = (status & 0x0800) === 0
      ? false
      : true;

    status &= 0x0f00;
    this.state.controller.io_faulted = status >> 4 === 2 ? true : false;
    this.state.controller.faulted = status >> 4 === 2
      ? true
   ...
   */
  readTag(tag: Tag | Structure, size: number | null = null): Promise<void> {
   ...
   */
  // Property 'forEach' in type 'Controller' is not assignable to the same property in base type 'ENIP'.
  // Type '(callback: (tag: Tag | Structure) => void | Promise<void>) => void' is not assignable to type '(fn: (data: any, options?: Pick<ArrayOptions, "signal"> | undefined) => void | Promise<void>, options?: ArrayOptions | undefined) => Promise<...>'.
  // Type 'void' is not assignable to type 'Promise<void>'.deno-ts(2416)
  // // @ts-ignore: Not sure why this is not a type

  override forEach(
    callback: (tag: Tag | Structure) => Promise<void> | void,
  ): Promise<void> {
    ...
    const data = await promiseTimeout(
      new Promise((resolve, reject) => {
        this.on("Read Tag", async (err, data) => {
          if (
            err && err.generalStatusCode !== 6 &&
            !(err.generalStatusCode === 255 &&
              err.extendedStatus.toString() === [8453].toString())
          ) {
            err.service += ":" + tag.name;
            reject(err);
            return;
          }

          if (
            err && err.generalStatusCode === 255 &&
            err.extendedStatus.toString() === [8453].toString()
          ) {
            tag.state.read_size--;
            this._readTag(tag).catch(reject);
          } else if (err && err.generalStatusCode === 6) {
            await this._readTagFragmented(tag, size as number).catch(reject);
            resolve(null);
          } else {
            resolve(data);
          }
  ...
  async _writeTag(
    ...
    if (tag.state.tag.type >= 0xc1 && tag.state.tag.type <= 0xca) {
      dataSize = CIP.DataTypes
        .TypeSizes[
          tag.state.tag.type as keyof typeof CIP.DataTypes.TypeSizes
        ];
    }
  ...
  async _writeTagFragmented(
    ...
    let valueFragment = tag.state.tag.value.slice(
      0,
      Math.floor(maxPacket / dataSize),
    );
    let MR = tag.generateWriteMessageRequestFrag(offset, valueFragment, size);
    this.write_cip(MR);
    let numWrites = 0;

    const totalWrites = Math.ceil(
      tag.state.tag.value.length * dataSize / maxPacket,
    );
    const writeTagErr = new Error(
      `TIMEOUT occurred while writing Writing Tag: ${tag.name}.`,
    );
        ...
        this.on("Write Tag Fragmented", (err, data) => {
          ...
          if (numWrites < totalWrites) {
            valueFragment = tag.state.tag.value.slice(
              Math.floor(offset / dataSize),
              Math.floor(maxPacket / dataSize) + Math.floor(offset / dataSize),
            );
      ...
      const data = await promiseTimeout(
        new Promise((resolve, reject) => {
          this.on("Multiple Service Packet", async (err, data) => {
            if (
              err && err.generalStatusCode !== 6 && err.generalStatusCode !== 4
            ) reject(err);
            for (let i = 0; i < data.length; i++) {
              if (data[i].generalStatusCode === 6) {
                await this._readTagFragmented(group.state.tags[msg.tag_ids[i]])
                  .catch(reject);
              }
            }
  ...
  async _writeTagGroup(group: TagGroup): Promise<void> {
    ...
    const writeTagGroupErr = new Error(
      "TIMEOUT occurred while Writing Tag Group.",
    );
    ...
    for (const msg of messages) {
      if (msg.data) {
        ...
        group.parseWriteMessageRequests(msg.tag_ids as string[]);
      } else {
        await this.writeTag(msg.tag as Tag | Structure).catch((e: any) => {
          throw e;
        });
      }
    }
  ...
  _handleSendRRDataReceived(srrd: CommonPacketData[]): void {
    const { service, generalStatusCode, extendedStatus, data } = CIP
      .MessageRouter.parse(
        srrd[1].data,
      );
    ...
    const serviceName = Object.entries(CIP.MessageRouter.services).find(
      ([_, value]) => value === service - 0x80,
    )?.[0];

    let error = generalStatusCode !== 0
      ? CIPError.getError(generalStatusCode, extendedStatus, serviceName)
      : null;
    ...
    switch (service - 0x80) {
      ...
      case MULTIPLE_SERVICE_PACKET: {
        ...
        const services = data.readUInt16LE(0);
        const offsets = [];
        const responses = [];
        ...
        for (let i = 0; i < offsets.length - 1; i++) {
          ...
          if (
            msgData.generalStatusCode !== 0 && error?.generalStatusCode !== 30
          ) {
            error = CIPError.getError(
              msgData.generalStatusCode,
              msgData.extendedStatus,
              serviceName,
            );
          }
  ...
  _handleSendUnitDataReceived(sud: CommonPacketData[]) {
    const sudnew = sud[1].data.subarray(2); // First 2 bytes are Connection sequence number
    const { service, generalStatusCode, extendedStatus, data } = CIP
      .MessageRouter.parse(
        sudnew,
      );
  ...
  newTag(
    ...
    arrayDims: number = 0,
    arraySize: number = 0x01,
  ): Tag | Structure {
    const template = this.state.tagList.getTemplateByTag(tagname, program);
    let tag = null;
    if (template) {
      tag = new Structure(
        tagname,
   ...
   * @returns
   */
  getTagByName(name: string): Tag | Structure | undefined {
    return Object.values(this.state.subs.state.tags).find((tag) =>
      tag.state.tag.name === name
    );
  }
}
