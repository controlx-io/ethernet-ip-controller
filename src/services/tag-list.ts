...
import { Template } from "./structure.ts";
import type Controller from "./controller.ts";
import { Buffer } from "node:buffer";
import { templateMember } from "./structure-template.ts";
...
class TagList {
  ...
  _generateListMessageRequest(
    ...
    const request = CIP.MessageRouter.build(
      CIP.MessageRouter.services.GET_INSTANCE_ATTRIBUTE_LIST,
      Buffer.concat(pathArray),
      requestData,
    );
   ...
   */
  get programs(): string[] {
    return this.tags.filter((tag) => tag.name.slice(0, 8) === "Program:").map(
      (tag) => {
  ...
  getTag(
    tagName: string,
    program: string | null = null,
  ): tagListTag | undefined {
    return this.tags.find((tag) =>
      tag.name === tagName && tag.program === program
    );
  }
  ...
  getTemplateByTag(
    ...
    const tag = this.tags.find((tag) =>
      tag.name.toLowerCase().replace(/\[.*/, "") ===
        tagArray[0].toLowerCase().replace(/\[.*/, "") &&
      String(tag.program).toLowerCase() === String(program).toLowerCase()
    );

    if (tag) {
      let finalTemplate: Template | null = this.templates[tag.type.code];
      let tagArrayPointer = 1;
      while (finalTemplate && tagArrayPointer < tagArray.length) {
        const memberName = String(tagArray[tagArrayPointer]).replace(
          /\[.*/,
          "",
        ); //removes array indication
        const nextTag: templateMember | undefined = finalTemplate?._members
    ...
    return new Promise(async (resolve, reject) => {
      for (const tag of this.tags) {
        if (tag.type.structure && !this.templates[tag.type.code]) {
          ...
          } catch (_) { /* ignore template fetching errors */ }
        }
      }
...
export type { tagListTag, tagListTagType, tagListTemplates };
